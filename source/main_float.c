...
Neuron InputLayer[INPUT_NEURON_NUMBER][DATA_SIZE];
Neuron HiddenLayer[HIDDEN_NEURON_NUMBER][DATA_SIZE];
Neuron OutputLayer[DATA_SIZE];
...
float Foreward_Prop(float setValue, float inputValue, int idx)
{
	...
	if (idx == 0)
	{
		HiddenLayer[NEURON_TYPE_I][idx].x = I_Activation_Func(HiddenLayer[NEURON_TYPE_I][idx].u, 0);
		HiddenLayer[NEURON_TYPE_D][idx].x = D_Activation_Func(HiddenLayer[NEURON_TYPE_D][idx].u, 0);
